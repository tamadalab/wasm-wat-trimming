...
# -*- coding: utf-8 -*-
"""
corr_compare_deterministic.py
------------------------
Deterministicトリミング前後の類似度行列CSVを読み込み、
その相関係数（Pearson r）を計算して評価するスクリプト。

使い方:
  python scripts/corr_compare_deterministic.py

前提条件:
- output/not_trimmed_matrices/ に Before の類似度行列CSV（6手法）が存在
- output/trimmed_matrices/ に After の類似度行列CSV（6手法）が存在
"""
...
METHODS = {
    "Cosine": {
        ...
        "after_csv": "cosine_similarity_matrix.csv",
    },
    "Jaccard": {
        "before_csv": "jaccard_similarity_matrix.csv",
        "after_csv": "jaccard_similarity_matrix.csv",
    },
    "Overlap": {
        "before_csv": "overlap_similarity_matrix.csv",
        "after_csv": "overlap_similarity_matrix.csv",
    },
    "Manhattan": {
        "before_csv": "manhattan_similarity_matrix.csv",
        "after_csv": "manhattan_similarity_matrix.csv",
    },
    "KL": {
        "before_csv": "kl_similarity_matrix.csv",
        "after_csv": "kl_similarity_matrix.csv",
    },
    "LCS": {
        "before_csv": "lcs_instruction_similarity_matrix_min.csv",
        "after_csv": "lcs_instruction_similarity_matrix_min.csv",
    },
}
...
def main():
    ...
    after_dir = Path("output/trimmed_matrices")
    ...
    if not after_dir.exists():
        ...
        print("先に類似度行列（After）を生成してください")
        print("実行: make trimmed-matrices")
        return 1
    
    results = []
    
    print("=" * 80)
    print("トリミング前後の類似度行列 相関係数比較 (Deterministic)")
    print("=" * 80)
    ...
    print("=" * 80)
    print("⭐ トリミング前後の類似度行列 相関係数まとめ (Deterministic)")
    print("=" * 80)
    ...
    save_path = results_dir / "correlation_stats_deterministic.csv"
